...
describe('WordsInput component', () => {
  it('when clicking "legg til et nytt ord", then one additional input is visible', async () => {
    ...
    const fieldsBefore = await screen.getAllByPlaceholderText(/skriv inn et ord/i);
    ...
    // Assert
    const fieldsAfter = await screen.getAllByPlaceholderText(/skriv inn et ord/i);
  ...
  it('when clicking "send inn ord", then calls onSubmit prop with words', async () => {
   ...
   render(
    ...
    userEvent.click(addFieldsButton);
    const fieldsAfter = await screen.getAllByPlaceholderText(/skriv inn et ord/i);
